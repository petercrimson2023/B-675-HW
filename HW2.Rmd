...
Assuming the $\hat{\Lambda} (t)$ follows normal distribution, and the variance of $\hat{\Lambda}(t)$ is estimated using Greenwood's formula as $\hat{V}(t)=\sum_{t_j<t}\frac{D_j}{Y_j^2}$, then the confidence interval is estimated as $\hat{\Lambda}(t) \pm z_{0.975}\sqrt{\hat{V}(t)}$.
...
Assuming the $\hat{\Lambda} (t)$ follows normal distribution.  The variance of $\hat{\Lambda (t)}$ , using Delta method is estimated as $\hat{V}(\log(\hat{\Lambda} (t)) = \sum_{t_j<t}\frac{D_j}{Y_j^2 \hat{\Lambda}(t)^2}$, then the confidence interval is estimated as $\exp(\log(\hat{\Lambda}(t)) \pm z_{0.975}\sqrt{\hat{V}(\log(\hat{\Lambda}(t)))}$.
